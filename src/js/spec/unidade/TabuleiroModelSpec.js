...
function(
  ...
  describe('TabuleiroModel', function() {
    ...
    describe('ao executar um movimento', function() {
      ...
      it('deve se modificar (por exemplo, eliminando itens coletados)', function() {
        var resultadoMovimento = tabuleiroModel.executarMovimento(ESQUERDA);

        expect(resultadoMovimento).toEqual(
          new ResultadoMovimentoBuilder(aPosicao).parteDe(0, 2).vaiPara(0, 0).eTermina()
        );
      });
    });
  });
